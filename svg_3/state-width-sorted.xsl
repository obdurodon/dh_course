...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.w3.org/2000/svg"
    ...
    <!-- $sortedStates as element(state)+ : sorted by dem percent         -->
    <!-- ================================================================ -->
    ...
    <xsl:variable name="sortedStates" as="element(state)+">
        <xsl:perform-sort select="//state">
            <xsl:sort select="candidate[@party eq 'Democrat'] div sum(candidate)" order="descending"
            />
        </xsl:perform-sort>
    </xsl:variable>
    ...
    <xsl:template match="/">
        <svg height="{$fullHeight + 200}">
            <g transform="translate(120, {$fullHeight + 30})">
                ...
                <!-- ==================================================== -->
                <xsl:apply-templates select="$sortedStates"/>
    ...
    <xsl:template match="state">
        ...
        <xsl:variable name="precedingStates" as="element(state)*"
            select="$sortedStates[position() lt $currentStatePos]"/>
